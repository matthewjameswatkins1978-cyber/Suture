...
Threadmoth 1.7.0 gives benchmark and torture runs a compact human presentation while preserving the existing structured `--json` report for scripts and agents. A human benchmark now ends with a one-line release signal such as:
