...
Recovery journals are validated for structure, supported version, safe transaction ID, workspace-contained member paths, duplicate paths, size limits, and SHA-256 consistency before recovery writes. Their location in `.threadmoth-recovery` or legacy `.suture-recovery` is not authenticated provenance: another same-user process with equivalent filesystem authority may plant or tamper with a journal. Recovery refuses when member state is not provably original or candidate.
