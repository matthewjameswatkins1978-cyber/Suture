...
`filesystem` is the canonical lifecycle-provider spelling in Threadmoth 1.7.1 discovery, schema output, certificates, and newly generated requests. The older request spelling `file` remains accepted as a compatibility alias and is canonicalized to `filesystem` when Threadmoth serializes it.
