...
A request is JSON with `version: "1.0.0"`, a stable `request_id`, a workspace-relative `file_path`, optional `namespace`, optional `expected_pre_hash`, a `cardinality`, an optional hard effect `budget`, and an operation. Run `suture schema` for the generated schema or `suture capabilities` for feature negotiation. Unknown fields are rejected.

The operation is encoded as an outer provider and nested tagged operation:

```json
{"version":"1.0.0","request_id":"example-2","file_path":"config.json","cardinality":{"type":"exactly_one"},"budget":{"max_files":1,"max_matches":1,"max_changed_lines":4},"operation":{"provider":"json","operation":{"type":"set","path":"$.server.port","value":8080}}}
```
