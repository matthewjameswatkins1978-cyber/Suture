...
A request is JSON with `version: "1.1.0"`, a stable `request_id`, a workspace-relative `file_path`, optional `namespace`, optional `expected_pre_hash`, a `cardinality`, an optional hard effect `budget`, and an operation. Run `suture help`, `suture examples`, `suture schema`, `suture explain`, `suture suggest`, or `suture capabilities` for local discovery. Unknown fields are rejected.
...
Every refusal and relevant failure certificate includes a stable `reason_code`; use `suture explain REASON_CODE` for local recovery guidance or `suture suggest --from-refusal CERTIFICATE` for deterministic next request skeletons. Exit codes: `0` applied/no-change, `2` refused, `3` runtime failure.
